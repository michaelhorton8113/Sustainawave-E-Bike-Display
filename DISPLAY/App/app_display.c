...
void MX_DISPLAY_Process(void)
{
  /* USER CODE BEGIN MX_DISPLAY_Process */
    ...
    UTIL_LCD_DisplayStringAt(5, 80, (uint8_t *)"Speed: ", LEFT_MODE);
    UTIL_LCD_DisplayStringAt(5 + strlen("Speed: ") * UTIL_LCD_GetFont()->Width, 80, (uint8_t *)buffer, LEFT_MODE);
    memset(buffer, 0, sizeof(buffer));

    snprintf(buffer, sizeof(buffer), "%d", assist_level);
    UTIL_LCD_SetFont(&Font20);
    UTIL_LCD_DisplayStringAt(5, 120, (uint8_t *)"Assist Level: ", LEFT_MODE);
    UTIL_LCD_DisplayStringAt(5 + strlen("Assist Level: ") * UTIL_LCD_GetFont()->Width, 120, (uint8_t *)buffer, LEFT_MODE);
    memset(buffer, 0, sizeof(buffer));

    snprintf(buffer, sizeof(buffer), "%d  %%", battery_level);
    UTIL_LCD_SetFont(&Font20);
    UTIL_LCD_DisplayStringAt(5, 160, (uint8_t *)"Battery Level: ", LEFT_MODE);
    UTIL_LCD_DisplayStringAt(5 + strlen("Battery Level: ") * UTIL_LCD_GetFont()->Width, 160, (uint8_t *)buffer, LEFT_MODE);
    memset(buffer, 0, sizeof(buffer));

    snprintf(buffer, sizeof(buffer), "%d W", available_power);
    UTIL_LCD_SetFont(&Font20);
    UTIL_LCD_DisplayStringAt(5, 200, (uint8_t *)"Available Power:", LEFT_MODE);
    UTIL_LCD_DisplayStringAt(5 + strlen("Available Power: ") * UTIL_LCD_GetFont()->Width, 200, (uint8_t *)buffer, LEFT_MODE);
    memset(buffer, 0, sizeof(buffer));
